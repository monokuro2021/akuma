...
function showChallenge() {
  // チャレンジモードを有効化
  challengeMode = true;
  challengeCurrentLevel = 4; // レベル4から開始
...
function showClearMessage() {
  ...
  // チャレンジモードの場合は「次のステージへ」ボタンを表示
  if (challengeMode) {
    messageDiv.innerHTML = `<button onclick="challengeNextStage()" style="
      background: #4ade80;
      color: white;
      border: none;
      padding: 1vw 2vw;
...
function challengeNextStage() {
  ...
  if (challengeCurrentLevel > challengeMaxLevel) {
    ...
    completeMessage.style.cssText = `
      ...
      z-index: 1000;
    `;
    completeMessage.innerHTML = `おめでとうございます！<br>全ステージクリア！<br><br><button onclick="backToStart()" style="
      background: #4ade80;
      color: white;
      border: none;
      padding: 1vw 2vw;
